...
async def async_setup_entry(hass, entry, async_add_entities):
    ...
    async def async_update_data():
        ...
        data = await get_snmp_data_map(engine, host, port, user, key, oids)
        if not data:
            _LOGGER.warning("ИБП %s не ответил на запрос", host)
            return None
        return data
    ...
    def native_value(self):
        ...
        try:
            ...
            if self._config[CONF_MAP]:
                return MAPS[self._config[CONF_MAP]].get(int(clean_val), f"Stat {clean_val}")
            return round(float(clean_val) / self._config[CONF_DIVISOR], 1)
        except:
            return str(raw)
